...
~> **Note:** When using aws_dynamodb_table_replica with this resource, use `lifecycle` `ignore_changes` for `replica`, _e.g._, `lifecycle { ignore_changes = [replica] }`.
...
~> **Note:** aws_dynamodb_table_replica is an alternate way of configuring Global Tables. Do not use `replica` configuration blocks of `aws_dynamodb_table` together with aws_dynamodb_table_replica.
