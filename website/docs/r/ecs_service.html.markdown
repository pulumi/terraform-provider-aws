...
* `wait_for_steady_state` - (Optional) If `true`, the provider will wait for the service to reach a steady state (like `aws ecs wait services-stable`) before continuing. Default `false`.
