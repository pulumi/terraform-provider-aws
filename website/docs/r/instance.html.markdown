...
* `placement_partition_number` - (Optional) Number of the partition the instance is in. Valid only if the `aws_placement_group` resource's `strategy` argument is set to `"partition"`.
